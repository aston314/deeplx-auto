...
// 全局配置
interface Config {
  DEEPLX_API_URL: string;
  BATCH_SIZE: number;
  SUBTITLE_SEPARATOR: string;
  SUBTITLE_MARKER: string;
  OPTIMAL_TEXT_LENGTH: number;
  ...
  NTFY_TOPIC: string;
  NTFY_URL: string;
  LANGUAGE_DETECTION_SAMPLE_SIZE: number;
        ...
        case "translate":
          ...
          if (targetLangCount >= detectionThreshold) {
            ...
            try {
              ...
              for (let i = 0; i < mergedSubtitles.length; i += config.BATCH_SIZE) {
                ...
                const translatedBatch = await Promise.all(batch.map(async (item) => {
                  ...
                  try {
                    ...
                    console.error(`[Translator] 翻译失败: ${error instanceof Error ? error.message : String(error)}`);
                    if (error instanceof Error && (error.name === 'AbortError' || error.message === "Translation stopped")) {
                      throw error;
                    }
              ...
              } else if (!(error instanceof Error && error.name === 'AbortError') && isConnected) {
                ws.send(JSON.stringify({ action: "error", message: error instanceof Error ? error.message : String(error) }));
              }
            } finally {
              isTranslating = false;
              abortController = null;
